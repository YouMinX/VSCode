...
> 链接其他文档
> 在 Markdown 里做链接，分两种需求：

1. 跳到**同目录下的另一个 `*.md` 文件**；
2. 跳到**该文件内部的某个标题/锚点**；
3. 跳到**另一个文件内部的某个标题/锚点**。

---

### 1️⃣ 链接到同目录的另一个 Markdown 文件
...
- `./` 表示“当前目录”，也可以省略。
- 文件扩展名 `.md` 建议保留，GitHub / Gitee / VS Code 都能识别。

---

### 2️⃣ 链接到**当前文件**内的某个标题

Markdown 会自动给每个标题生成锚点，规则大致是：
...
### 3️⃣ 链接到**另一个文件**内的某个标题

把上面两种语法拼在一起即可：

```markdown
[查看文档 B 的“安装”小节](./docB.md#安装)
```

- `#安装` 就是目标文件里 `## 安装` 自动生成的锚点。
- 如果标题里有空格、标点，规则同上（小写、空格变 `-`）。
...
| 同目录文件     | `[文本](./file.md)`                    |
| 当前文件内标题 | `[文本](#标题)`                        |
| 其他文件内标题 | `[文本](./file.md#标题)`               |
| 自定义锚点     | `<a id="xxx"></a>` 然后 `[文本](#xxx)` |

把文件放在同一仓库或同一目录，上述写法在 GitHub、Gitee、Typora、VS Code 都能正常跳转。
